...
async def auto_filter(bot: Client, update: Message):
    ...
    if filters:
        results.append(
                [
                    InlineKeyboardButton("💢𝘾𝙃𝘼𝙉𝙉𝙀𝙇💢", url="https://t.me/joinchat/nAJOyC9EPe5kMTBl"),
                    InlineKeyboardButton("⭕️𝙇𝙄𝙉𝙆𝙎⭕️", url="https://t.me/Cinemafactory_all"),
                ]
            )
    ...
    if len(results) == 0: # double check
        ...
        try:
            await bot.send_message(
                chat_id = update.chat.id,
                text=f"<b>🌟𝐑𝐞𝐪𝐮𝐞𝐬𝐭𝐞𝐝 𝐁𝐲:{message.from_user.mention}<b>\n\n🌩 𝐓𝐨𝐭𝐚𝐥 𝐑𝐞𝐬𝐮𝐥𝐭𝐬: {(len_results)} </b>\n<b>🎬 𝐌𝐨𝐯𝐢𝐞/𝐒𝐞𝐫𝐢𝐞𝐬:</b> <code>{query}</code>",
